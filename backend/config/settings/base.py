"""
Shared Django settings. `dev.py` and `prod.py` import * from here and layer
environment-specific overrides on top. See backend/docs/API_CONTRACT.md for
...
REST_FRAMEWORK = {
    ...
    "EXCEPTION_HANDLER": "config.exceptions.hms_exception_handler",
}

# --- SimpleJWT ---------------------------------------------------------------
# Access token payload must carry user_id/exp/token_type — see auth.model.ts
# decodeAccessToken(), which the frontend relies on directly.
SIMPLE_JWT = {
...
SPECTACULAR_SETTINGS = {
    ...
    "DESCRIPTION": "Drop-in Django REST backend for the hospital-management-angular frontend.",
    "VERSION": "1.0.0",
